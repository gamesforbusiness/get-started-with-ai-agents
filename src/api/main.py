...
async def lifespan(app: fastapi.FastAPI):
    ...
    try:
        ...
        async with (
            ...
            logger.info(f"Storage config: AZURE_STORAGETABLE_RESOURCEENDPOINT='{storage_endpoint}', AZURE_STORAGE_ACCOUNT_NAME='{storage_account_name}'")
            table_endpoint = ""
            if storage_endpoint:
                table_endpoint = storage_endpoint
            ...
            if table_endpoint:
                ...
                table_credential = ManagedIdentityCredential()
                conv_mgr = ConversationManager(table_endpoint, table_credential)
                app.state.conversation_manager = conv_mgr
                logger.info(f"Initialized ConversationManager with System Assigned MI, endpoint: {conv_mgr.account_url}")
            else:
                app.state.conversation_manager = None
                logger.info("No AZURE_STORAGETABLE_RESOURCEENDPOINT or AZURE_STORAGE_ACCOUNT_NAME set, chat history disabled")
